...
function PersonalModal({ miembro, businessId, orgId, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      ...
      <div className="relative w-full max-w-lg bg-[var(--dash-ink-sunken)] border border-[#222] rounded-2xl shadow-2xl shadow-black/50 overflow-hidden max-h-[90vh] overflow-y-auto">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--dash-border)] sticky top-0 bg-[var(--dash-ink-sunken)] z-10">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-[var(--dash-text-dim)] text-[10px]">PNG, JPG o WEBP · máx 2 MB</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <p className="text-[var(--dash-text-muted)] text-[10px] uppercase tracking-widest">Teléfono</p>
              <input
                type="text"
          ...
          {/* Bio */}
          <div className="space-y-1">
            <p className="text-[var(--dash-text-muted)] text-[10px] uppercase tracking-widest">Bio corta <span className="normal-case text-[var(--dash-text-dim)]">(opcional · visible al cliente)</span></p>
            <textarea
              value={form.bio}
            ...
            />
            <p className="text-[10px] text-right text-[var(--dash-text-dim)] tabular-nums">{form.bio.length}/300</p>
          </div>

          {/* Especialidades */}
          <div className="space-y-2">
            <p className="text-[var(--dash-text-muted)] text-[10px] uppercase tracking-widest">Especialidades <span className="normal-case text-[var(--dash-text-dim)]">(hasta 8)</span></p>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {specialties.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 text-xs bg-[var(--dash-primary)]/10 text-[var(--dash-primary)] border border-[var(--dash-primary)]/30 px-2.5 py-1 rounded-full">
                    {t}
                    <button type="button" onClick={() => removeSpecialty(t)} className="text-[var(--dash-primary)]/60 hover:text-[var(--dash-primary)]" aria-label="Quitar">
                      ×
                    </button>
                  </span>
                ))}
          ...
          {/* Rol */}
          <div className="space-y-1">
            <p className="text-[var(--dash-text-muted)] text-[10px] uppercase tracking-widest">Rol</p>
          ...
          {/* Horario semanal */}
          <div className="space-y-3">
            <p className="text-[var(--dash-text-muted)] text-[10px] uppercase tracking-widest">Horario semanal</p>
            <div className="space-y-2">
              {DIAS.map(({ key, label }) => {
                  ...
                  <div key={key} className={`rounded-xl border transition-colors ${active ? 'border-[var(--dash-border)] bg-[var(--dash-ink-raised)]' : 'border-[var(--dash-border)] bg-transparent'}`}>
                    <div className="flex items-center gap-3 px-4 py-2.5">
                      <button
                        ...
                        className={`relative w-8 h-5 rounded-full transition-colors shrink-0 ${active ? 'bg-[var(--dash-primary)]' : 'bg-[var(--dash-border)]'}`}
                      >
          ...
          <div className="flex items-center justify-between py-1">
            <div>
              <p className="text-[var(--dash-text-soft)] text-sm">Colaborador activo</p>
              <p className="text-[var(--dash-text-muted)] text-xs mt-0.5">Aparece disponible para asignar citas</p>
            </div>
            <button
              type="button"
...
function Avatar({ name, photoUrl }) {
  ...
  if (photoUrl) {
    return (
      // eslint-disable-next-line @next/next/no-img-element
      <img src={photoUrl} alt={name} className="w-10 h-10 rounded-xl object-cover border border-[var(--dash-border)] shrink-0" />
    )
  }
  return (
    <div className="w-10 h-10 rounded-xl bg-[var(--dash-primary)]/10 border border-[var(--dash-primary)]/20 flex items-center justify-center shrink-0">
      <span className="text-[var(--dash-primary)] text-sm font-semibold">{initials}</span>
    </div>
  )
}
...
function HorarioChips({ schedule }) {
  ...
  if (activos.length === 0) return <p className="text-[var(--dash-text-dim)] text-xs">Sin horario</p>
  return (
    <div className="flex gap-1 flex-wrap">
      {activos.map(({ key, label }) => (
        <span key={key} className="text-[10px] text-[var(--dash-text-dim)] border border-[var(--dash-border)] px-1.5 py-0.5 rounded">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
          ...
          <div className="relative bg-[var(--dash-ink-sunken)] border border-[#222] rounded-2xl shadow-2xl p-6 w-full max-w-sm space-y-4">
            <h3 className="text-[var(--dash-text)] text-base font-medium">Eliminar colaborador</h3>
            <p className="text-[var(--dash-text-dim)] text-sm">
              ¿Eliminar a <span className="text-[var(--dash-text-soft)]">{deleteConfirm.name}</span>? Esta acción no se puede deshacer.
            </p>
