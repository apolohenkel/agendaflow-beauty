...
// Colores oficiales del brand AgendaFlow — se usan en el header del email.
// El body del mail sigue usando el primary de la vertical del tenant.
const BRAND_NAVY = '#17384A'
const BRAND_GOLD = '#C8A263'

function layout({ title, body, vertical }) {
  const t = themeForEmail(vertical)
  // Logo del brand como SVG inline (soportado por Gmail/Apple Mail/Yahoo;
  // los clientes sin soporte caerán al wordmark de texto justo al lado).
  const logoSvg = `<svg width="36" height="36" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;display:inline-block;" aria-hidden="true">
    <path d="M 12 82 L 28 18 L 44 82" fill="none" stroke="${BRAND_NAVY}" stroke-width="6.5" stroke-linecap="round" stroke-linejoin="round"/>
    <line x1="19.5" y1="60" x2="36.5" y2="60" stroke="${BRAND_NAVY}" stroke-width="5" stroke-linecap="round"/>
    <line x1="54" y1="18" x2="54" y2="82" stroke="${BRAND_NAVY}" stroke-width="6.5" stroke-linecap="round"/>
    <line x1="54" y1="18" x2="78" y2="18" stroke="${BRAND_NAVY}" stroke-width="5.5" stroke-linecap="round"/>
    <path d="M 54 46 L 70 46 C 82.5 46 90 40 92 31" fill="none" stroke="${BRAND_GOLD}" stroke-width="5" stroke-linecap="round"/>
  </svg>`
  return `<!DOCTYPE html>
<html lang="es">
<head>
...
<body style="margin:0;padding:0;background:#f5f5f5;">
  <div style="font-family:'Onest','Inter',system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;padding:32px 16px;color:${t.text};background:#ffffff;">
    <div style="display:flex;align-items:center;gap:10px;padding:4px 0;border-bottom:2px solid ${BRAND_GOLD};margin-bottom:24px;">
      ${logoSvg}
      <span style="font-family:'Onest','Inter',system-ui,sans-serif;font-size:22px;font-weight:600;color:${BRAND_NAVY};letter-spacing:-0.5px;">Agenda<span style="color:${BRAND_GOLD};">Flow</span></span>
    </div>
    <h1 style="font-family:Fraunces,Georgia,serif;font-size:30px;font-weight:400;margin:0 0 16px;color:${t.text};line-height:1.2;">${title}</h1>
    ${body}
    <hr style="border:none;border-top:1px solid ${t.border};margin:32px 0 16px" />
    <p style="color:${t.textMuted};font-size:11px;margin:0;">AgendaFlow Beauty · <a href="${APP_URL}" style="color:${BRAND_GOLD};text-decoration:none;">${APP_URL.replace(/^https?:\/\//, '')}</a></p>
  </div>
</body>
</html>`
}
