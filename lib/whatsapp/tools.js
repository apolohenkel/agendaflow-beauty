...
export async function execTool({ name, input, ctx, admin }) {
  ...
  switch (name) {
    ...
    case 'get_business_info': {
      const { data: biz } = await admin
        .from('businesses')
        .select('name, address, timezone, currency, opening_hours, whatsapp_number, deposit_enabled, deposit_currency')
        .eq('id', businessId)
        .maybeSingle()
    ...
    case 'create_appointment': {
      ...
      const { data: biz } = await admin
        .from('businesses')
        .select('name, currency, deposit_enabled, deposit_currency, organization_id')
        .eq('id', businessId)
        .maybeSingle()
      if (!biz) return { error: 'negocio_no_encontrado' }
