  ...
  switch (name) {
    ...
    case 'get_business_info': {
      const { data: biz } = await admin
        .from('businesses')
        .select('name, address, timezone, opening_hours, whatsapp_number, deposit_enabled, deposit_currency')
        .eq('id', businessId)
        .single()
      if (!biz) return {}
      ...
      if (biz.deposit_enabled) {
        ...
        if (svcs && svcs.length > 0) {
          ...
          const curr = (biz.deposit_currency || 'usd').toUpperCase()
          deposit_summary = min === max
            ? `Algunos servicios requieren seña de ${(min / 100).toFixed(2)} ${curr}.`
            : `Algunos servicios requieren seña entre ${(min / 100).toFixed(2)} y ${(max / 100).toFixed(2)} ${curr}.`
        }
      }
    ...
    case 'create_appointment': {
      ...
      const { data: biz } = await admin
        .from('businesses')
        .select('name, deposit_enabled, deposit_currency, organization_id')
        .eq('id', businessId)
        .single()
      ...
      if (totalDeposit > 0 && orgSlug) {
        try {
          ...
          return {
            ...
            deposit_amount: `${(totalDeposit / 100).toFixed(2)} ${biz.deposit_currency?.toUpperCase() || 'USD'}`,
